def ductility(strength_ratio: float, mu: float, period: float, case: str, period_c: float) -> float:
	...
	if case.lower() == "fd":
		...
		raise ValueError("Case must be 'fd', 'in', 'sd', for details refer to https://doi.org/10.1002/eqe.2862")

	b = 2.3
	c = 2.1

	return abs(mu - (strength_ratio + pow(strength_ratio - 1, c) / ((period / period_hyst + ahyst) * pow(period / period_c, b))))
